...
public class Player {
    ...
    public static Player initializeCharacterStats(String name, Scanner myObj, DelayedStringPrinter narrator) throws InterruptedException{
        List<String> array = new ArrayList<String>(Arrays.asList("Attack", "Defense", "Health", "Stamina"));
		...
		while (true) {
			...
			for (int i = 0; i < array.size(); i++) {
				...
				narrator.print("You have " + addStats + " free points left to assign. How many points do you want to assign to " + array.get(i) + "?");
				int value = myObj.nextInt();
				builderInputs.set(i, value);
				addStats -= value;
				if (addStats < 0) {
					narrator.print("You spent " + addStats*-1 + " more points than you had to spend. Please try again.");
					break;
				}
			}
			
			if (addStats == 0) {
				Player player = new Player.Builder(name).attack(builderInputs.get(0)).defense(builderInputs.get(1)).health(builderInputs.get(2)).stamina(builderInputs.get(3)).build();
				return player;
			} else if ( addStats > 0) {
				narrator.print("You still have " + addStats + " free points left to assign. Do you want to re-assign these points?");
			} else {
				Thread.sleep(1000);
			}
		}
    ...
    public void levelUpUpdateStats(Player player, Scanner myObj, DelayedStringPrinter narrator, int addStats) throws InterruptedException {
        List<String> array = new ArrayList<String>(Arrays.asList("Attack", "Defense", "Health", "Stamina"));
		List<Integer> builderInputs = new ArrayList<Integer>(Arrays.asList(0,0,0,0));
        ...
        while (true) {
			...
			if (addStats == 0) {
				player = new Player.Builder(name).attack(builderInputs.get(0)).defense(builderInputs.get(1)).health(builderInputs.get(2)).stamina(builderInputs.get(3)).build();
